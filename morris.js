...
(function() {
  ...
  Morris.Bar = (function(_super) {
    ...
    Bar.prototype.drawSeriesLabels = function() {
      var box, i, info, label, labelText, prevLabel, prevLabels, rad, text, twoPrevLabel, xCenter, _i, _ref, _results;
      prevLabels = [];
      prevLabel = null;
      twoPrevLabel = null;
      _results = [];
      for (i = _i = 0, _ref = this.barInfo.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        info = this.barInfo[i];
        ...
        text = this.raphael.text(xCenter, info.yPos, labelText).attr('opacity', 0);
        box = text.getBBox();
        ...
        if (prevLabel && (prevLabels.filter((function(l) {
          return checkLabelsIntersect(label, l);
        }))).length) {
        ...
        this.raphael.circle(label.x, label.y, label.r).attr({
          fill: 'white',
          stroke: info.barColor,
          'stroke-width': 1
        });
        _results.push(this.raphael.text(label.x, label.y, labelText));
      }
      return _results;
    };

    checkLabelsIntersect = function(labelA, labelB) {
      var distance;
            ...
            for (sidx = _k = 0, _len1 = _ref2.length; _k < _len1; sidx = ++_k) {
              ...
              if (ypos !== null) {
                ...
                if (this.options.verticalGridCondition && this.options.verticalGridCondition(row.x)) {
                  if (!this.options.horizontal) {
                    this.drawBar(this.xStart + idx * groupWidth, this.yEnd, groupWidth, this.ySize, this.options.verticalGridColor, this.options.verticalGridOpacity, this.options.barRadius, row.y[sidx]);
                  } else {
                    this.drawBar(this.yStart, this.xStart + idx * groupWidth, this.ySize, groupWidth, this.options.verticalGridColor, this.options.verticalGridOpacity, this.options.barRadius, row.y[sidx]);
                  }
                }
                if (this.options.stacked) {
                  top -= lastTop;
                ...
                if (!this.options.horizontal) {
                  lastTop += size;
                  _results1.push(this.drawBar(left, top, barWidth, size, this.colorFor(row, sidx, 'bar'), this.options.barOpacity, this.options.barRadius, row.y[sidx]));
                } else {
                  lastTop -= size;
                  this.drawBar(top, left, size, barWidth, this.colorFor(row, sidx, 'bar'), this.options.barOpacity, this.options.barRadius, row.y[sidx]);
    ...
    Bar.prototype.drawBar = function(xPos, yPos, width, height, barColor, opacity, radiusArray, y) {
      var maxRadius, path;
      this.barInfo.push({
        xPos: xPos,
        yPos: yPos,
